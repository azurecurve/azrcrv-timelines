...
if ( is_array( $plugin_array ) ) {
	foreach ( $plugin_array as $plugin_name => $plugin_details ) {
		if ( 0 == $plugin_details['retired'] ) {
			$alternative_color = '';
			if ( isset( $plugin_details['bright'] ) && 1 == $plugin_details['bright'] ) {
				$alternative_color = 'bright-';
			}
			if ( isset( $plugin_details['premium'] ) && 1 == $plugin_details['premium'] ) {
...
$tab_plugins_label = esc_html__( 'Other Plugins', 'azrcrv-t' );
$tab_plugins        = '
<table class="form-table azrcrv-t-settings">
	...
	<tr>
		...
		<td scope="row" colspan=2>
			...
			<p>' .
				sprintf( esc_html__( '%1$s was one of the first plugin developers to start developing for ClassicPress; all plugins are available from %2$s and are integrated with the %3$s plugin for fully integrated, no hassle, updates.', 'azrcrv-t' ), '<strong>' . DEVELOPER_SHORTNAME . '</strong>', DEVELOPER_LINK, '<a href="https://directory.classicpress.net/plugins/update-manager/">Update Manager</a>' )
			. '</p>
			<p>' .
				sprintf( esc_html__( 'Other plugins available from %s, which you are not using, are:', 'azrcrv-t' ), '<strong>' . DEVELOPER_NAME . '</strong>' )
			. '</p>
